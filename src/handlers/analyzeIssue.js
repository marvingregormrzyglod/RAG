...
import { buildContextAnalysisPrompt } from '../services/promptBuilder';
import { publishAppEvent } from '../services/appEvents';

export default async ({ payload, context = {} }) => {
  const logger = new ExecutionLogger('ContextAnalysis');
  ...
  try {
    ...
    logger.log('Building context analysis prompt');
    ...
    logger.log('Submitting OpenAI background job for context analysis');
